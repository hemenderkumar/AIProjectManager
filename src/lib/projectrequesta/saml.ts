...
import type { prOrganizations } from "../db/schema";
...
// prOrganizations.samlEntityId/samlIdpMetadataUrl/samlIdpCert columns (see schema.ts).
//
...
export function samlEntityId(requestUrl: string) {
  return `${baseUrl(requestUrl)}/api/projectrequesta/saml/metadata`;
}

export function samlAcsUrl(requestUrl: string) {
  return `${baseUrl(requestUrl)}/api/projectrequesta/saml/acs`;
}

// The one shared SP. wantAssertionsSigned is left at samlify's default (true) -- we require
// the IdP to sign assertions, since ProjectRequesta is deciding account access based on the
...
type OrgSamlConfig = Pick<typeof prOrganizations.$inferSelect, "samlEntityId" | "samlIdpMetadataUrl" | "samlIdpCert">;
