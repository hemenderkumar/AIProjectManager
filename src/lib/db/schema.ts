...
export const tasks = pgTable("tasks", {
  ...
  // project via the cross-app bridge (see /api/projects/[id]/tasks/[taskId]/post-to-projectrequesta).
  // ProjectRequesta is now a fully separate app with its own database, so this can no longer be
  // a foreign key into pr_projects (that table doesn't exist in this database anymore) -- these
  // are just plain, unvalidated values: postedToMarketplaceProjectId is the id the bridge
  // endpoint's response returned (used only to build an external link to view it there), and
  // postedToMarketplaceAt shows "Posted to ProjectRequesta" instead of the action and prevents
  // posting the same task twice.
  postedToMarketplaceProjectId: text("posted_to_marketplace_project_id"),
  postedToMarketplaceAt: timestamp("posted_to_marketplace_at"),
});
...
export const auditLog = pgTable("audit_log", {
  ...
  createdAt: timestamp("created_at").notNull().defaultNow(),
  // JSON snapshots for entity state changes (e.g. via the generic AI-edit-via-chat flow),
  // per the "immutable ... before/after values" requirement -- the free-text `detail` above
  // isn't structured enough for that.
