      ...
      <div className="flex gap-1 border-b border-slate-200 mb-4 overflow-x-auto">
        <button
          ...
          className={`px-3 py-2 text-xs font-medium whitespace-nowrap border-b-2 -mb-px ${
            selectedSprintId === "backlog" ? "border-accent-600 text-accent-600" : "border-transparent text-slate-500 hover:text-slate-800"
          }`}
        >
        ...
        {sprints.map((s) => (
          <button
            ...
            className={`px-3 py-2 text-xs font-medium whitespace-nowrap border-b-2 -mb-px ${
              selectedSprintId === s.id ? "border-accent-600 text-accent-600" : "border-transparent text-slate-500 hover:text-slate-800"
            }`}
          >
                  ...
                  .map((t) => (
                    <div key={t.id} className="bg-white border border-slate-200 rounded-lg p-2">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        {t.storyPoints != null && (
                          <span className="text-xs px-1.5 py-0.5 rounded bg-accent-50 text-accent-600 font-medium">{t.storyPoints} pts</span>
                        )}
                      </div>
                      <select
                        value={t.status}
        ...
        <div className="mt-4">
          ...
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={velocityData}>
              ...
              <Bar dataKey="points" name="Story points completed" fill="var(--accent-600)" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
