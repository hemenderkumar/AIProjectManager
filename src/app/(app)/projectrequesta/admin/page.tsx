...
type ComplianceRow = { id: string; type: string; status: string; prOrganizationId: string; organizationName: string };
...
export default function ProjectRequestaAdminPage() {
  ...
  async function load() {
    ...
    const [complianceRes, orgsRes, disputesRes] = await Promise.all([
      fetch("/api/projectrequesta/admin/compliance"),
      fetch("/api/projectrequesta/organizations"),
      fetch("/api/projectrequesta/disputes"),
    ]);
  ...
  async function toggleOrgActive(o: Organization) {
    setOrgActionId(o.id);
    await fetch(`/api/projectrequesta/organizations/${o.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  async function deleteOrg(orgId: string) {
    ...
    const res = await fetch(`/api/projectrequesta/organizations/${orgId}`, { method: "DELETE" });
    setOrgActionId(null);
    if (!res.ok) {
    ...
    <div>
      ...
      <div className="p-8 max-w-4xl space-y-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200/70 shadow-sm shadow-slate-200/60 p-5">
          ...
          <div className="space-y-2">
            {openDisputes.map((d) => (
              <Link key={d.id} href="/projectrequesta/disputes" className="block text-sm text-slate-700 border-b border-slate-50 pb-2 last:border-0 hover:text-accent-600">
                {d.description}
              </Link>
            ))}
            ...
            {orgs.map((o) => (
              <div key={o.id} className={`border-b border-slate-50 pb-2 last:border-0 ${o.isActive ? "" : "opacity-60"}`}>
                <div className="flex items-center justify-between text-sm">
                  <Link href={`/projectrequesta/organizations/${o.id}`} className="flex items-center gap-2 hover:text-accent-600">
                ...
                <AiEditChat
                  entityType="prOrganizationAdmin"
                  entityId={o.id}
                  onApplied={() => load()}
