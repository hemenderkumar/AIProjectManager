...
type AgreementParty = { partyRole: "CLIENT" | "VENDOR" | "PLATFORM"; prOrganizationId: string | null };
type Agreement = { id: string; type: string; status: string; parties: AgreementParty[] };
type Milestone = {
  id: string;
  prAgreementId: string;
  description: string;
  amount: number;
  currency: string;
  dueDate: string | null;
...
type Payment = { id: string; prMilestoneId: string; amount: number; currency: string; direction: string; status: string };
type ChangeRequest = {
  id: string;
  prAgreementId: string;
...
type Me = { isPlatform: boolean; memberships: { prOrganizationId: string | null; role: string }[] };

// The underlying status value is still "SIGNED" (see prAgreementStatusEnum / VALID_TRANSITIONS
// in api/projectrequesta/agreements/[agreementId]/route.ts) -- only the label changes here. There's
...
export default function ProjectRequestaProjectDetailPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  async function load() {
    ...
    const [projectRes, orgsRes, bidsRes, agreementsRes, meRes, reviewsRes, sourceTaskRes] = await Promise.all([
      fetch(`/api/projectrequesta/projects/${projectId}`),
      fetch("/api/projectrequesta/organizations"),
      fetch(`/api/projectrequesta/projects/${projectId}/bids`),
      fetch(`/api/projectrequesta/projects/${projectId}/agreements`),
      fetch("/api/projectrequesta/me"),
      fetch(`/api/projectrequesta/projects/${projectId}/reviews`),
      fetch(`/api/projectrequesta/projects/${projectId}/source-task`),
    ]);
  ...
  async function loadNegotiations(bidId: string) {
    const res = await fetch(`/api/projectrequesta/bids/${bidId}/negotiations`);
    if (res.ok) {
  ...
  const myOrgIds = me?.memberships.filter((m) => m.prOrganizationId).map((m) => m.prOrganizationId as string) ?? [];
  ...
  function rolesInOrg(orgId: string | null): string[] {
    if (!orgId) return [];
    return (me?.memberships ?? []).filter((m) => m.prOrganizationId === orgId).map((m) => m.role);
  }
  function agreementOrgIds(a: Agreement) {
    return {
      clientOrgId: a.parties.find((p) => p.partyRole === "CLIENT")?.prOrganizationId ?? null,
      vendorOrgId: a.parties.find((p) => p.partyRole === "VENDOR")?.prOrganizationId ?? null,
    };
  }
  ...
  async function updateProjectStatus(status: string) {
    setError(null);
    const res = await fetch(`/api/projectrequesta/projects/${projectId}`, {
      method: "PATCH",
  ...
  async function submitBid() {
    ...
    setError(null);
    const res = await fetch(`/api/projectrequesta/projects/${projectId}/bids`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async function decideBid(bidId: string, status: "ACCEPTED" | "REJECTED") {
    setError(null);
    const res = await fetch(`/api/projectrequesta/bids/${bidId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ status }),
    });
  ...
  async function sendCounter(bidId: string) {
    ...
    await fetch(`/api/projectrequesta/bids/${bidId}/negotiations`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async function loadMilestones(agreementId: string) {
    const res = await fetch(`/api/projectrequesta/agreements/${agreementId}/milestones`);
    if (!res.ok) return;
  ...
  async function loadPayments(milestoneId: string) {
    const res = await fetch(`/api/projectrequesta/milestones/${milestoneId}/payments`);
    if (res.ok) {
      const data = await res.json();
  ...
  async function loadChangeRequests(agreementId: string) {
    const res = await fetch(`/api/projectrequesta/agreements/${agreementId}/change-requests`);
    if (res.ok) {
      const data = await res.json();
  ...
  async function decideChangeRequest(changeRequestId: string, agreementId: string, status: "ACCEPTED" | "REJECTED") {
    setError(null);
    const res = await fetch(`/api/projectrequesta/change-requests/${changeRequestId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ status }),
    });
  ...
  async function updateAgreementStatus(agreementId: string, status: string) {
    setError(null);
    const res = await fetch(`/api/projectrequesta/agreements/${agreementId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ status }),
    });
  ...
  async function approveMilestone(milestone: Milestone) {
    setError(null);
    const res = await fetch(`/api/projectrequesta/milestones/${milestone.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
    ...
    loadMilestones(milestone.prAgreementId);
  }

  async function raisePayment(milestone: Milestone, direction: string) {
    setError(null);
    const res = await fetch(`/api/projectrequesta/milestones/${milestone.id}/payments`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async function payWithStripe(payment: Payment) {
    setError(null);
    const res = await fetch(`/api/projectrequesta/payments/${payment.id}/checkout`, { method: "POST" });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) {
  ...
  async function updatePaymentStatus(payment: Payment, status: string) {
    setError(null);
    const res = await fetch(`/api/projectrequesta/payments/${payment.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ status }),
    });
    ...
    loadPayments(payment.prMilestoneId);
    if (status === "RELEASED") {
      const milestone = Object.values(milestonesByAgreement).flat().find((m) => m.id === payment.prMilestoneId);
      if (milestone) loadMilestones(milestone.prAgreementId);
    }
  }

  async function submitReview() {
    setError(null);
    const res = await fetch(`/api/projectrequesta/projects/${projectId}/reviews`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div>
      ...
      <div className="p-8 max-w-3xl space-y-6">
        ...
        {sourceTask && (
          <div className="bg-white rounded-xl border border-slate-200/70 shadow-sm shadow-slate-200/60 p-5">
            <p className="text-sm font-semibold text-slate-900 mb-1">Linked from Executa</p>
          ...
          <div className="bg-white rounded-xl border border-slate-200/70 shadow-sm shadow-slate-200/60 p-5">
            ...
            <div className="space-y-2">
              {agreements.map((a) => {
                ...
                const myOrgIdsForAgreement = a.parties.map((p) => p.prOrganizationId).filter((id): id is string => !!id && myOrgIds.includes(id));
                return (
                    ...
                    {expandedAgreement === a.id && (
                      <div className="border-t border-slate-100 p-3 space-y-3">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <a
                            href={`/api/projectrequesta/agreements/${a.id}/pdf`}
