...
export default function ProjectRequestaOrgDetailPage({ params }: { params: Promise<{ orgId: string }> }) {
  ...
  async function load() {
    ...
    const [orgRes, membersRes, complianceRes, meRes] = await Promise.all([
      fetch(`/api/projectrequesta/organizations/${orgId}`),
      fetch(`/api/projectrequesta/organizations/${orgId}/members`),
      fetch(`/api/projectrequesta/organizations/${orgId}/compliance`),
    ...
    if (orgRes.ok) {
      ...
      if (orgData.orgType === "VENDOR") {
        const stripeRes = await fetch(`/api/projectrequesta/organizations/${orgId}/stripe`);
  ...
  async function connectStripe() {
    ...
    setStripeError(null);
    const res = await fetch(`/api/projectrequesta/organizations/${orgId}/stripe`, { method: "POST" });
  ...
  const myRolesHere = me?.memberships.filter((m) => m.prOrganizationId === orgId).map((m) => m.role) ?? [];
  ...
  async function addMember() {
    ...
    setError(null);
    const res = await fetch(`/api/projectrequesta/organizations/${orgId}/members`, {
      method: "POST",
  ...
  async function removeMember(memberId: string) {
    await fetch(`/api/projectrequesta/organizations/${orgId}/members`, {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
  ...
  async function submitCompliance() {
    ...
    await fetch(`/api/projectrequesta/organizations/${orgId}/compliance`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async function decideCompliance(recordId: string, status: "VERIFIED" | "REJECTED") {
    await fetch(`/api/projectrequesta/compliance/${recordId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
        ...
        {org.orgType === "VENDOR" && isOrgAdmin && (
          <div className="bg-white rounded-xl border border-slate-200/70 shadow-sm shadow-slate-200/60 p-5">
            ...
            <p className="text-xs text-slate-500 mb-3">
              ...
              milestone. Executa never sees or holds your banking details directly — Stripe does.
            </p>
        ...
        {isOrgAdmin && (
          <div className="bg-white rounded-xl border border-slate-200/70 shadow-sm shadow-slate-200/60 p-5">
            ...
            <p className="text-xs text-slate-500 mb-4">
              Grant a role to an existing Executa account (they must already have a Executa login).
            </p>
        ...
        {isOrgAdmin && org.orgType === "CLIENT" && (
          <div className="bg-white rounded-xl border border-slate-200/70 shadow-sm shadow-slate-200/60 p-5 space-y-3">
            ...
            <div className="text-xs text-slate-500 space-y-1 bg-slate-50 rounded-lg p-3">
              <p><span className="font-medium text-slate-700">SP entity ID / metadata:</span> /api/projectrequesta/saml/metadata</p>
              <p><span className="font-medium text-slate-700">ACS URL:</span> /api/projectrequesta/saml/acs</p>
              <p><span className="font-medium text-slate-700">Login URL:</span> /api/projectrequesta/saml/login/{orgId}</p>
            </div>
            <div className="flex items-center gap-2">
              <input
