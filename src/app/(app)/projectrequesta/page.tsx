...
export default function ProjectRequestaHome() {
  ...
  useEffect(() => {
    async function load() {
      const [orgsRes, projectsRes, mfaRes] = await Promise.all([
        ...
        fetch("/api/projectrequesta/projects"),
        fetch("/api/projectrequesta/mfa/status"),
      ]);
  ...
  if (loading) {
    return (
      <div>
        <Topbar title="ProjectRequesta" subtitle="B2B marketplace for global IT project outsourcing" />
      ...
      <div className="p-8 max-w-4xl space-y-6">
        ...
        {orgs.length === 0 && (
          <div className="bg-white rounded-xl border border-slate-200/70 shadow-sm shadow-slate-200/60 p-6 text-center">
            <p className="text-sm font-semibold text-slate-900 mb-1">Get started on ProjectRequesta</p>
        ...
        {orgs.length > 0 && (
          <div className="bg-white rounded-xl border border-slate-200/70 shadow-sm shadow-slate-200/60 p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <Link href="/projectrequesta/organizations" className="text-xs text-accent-600 hover:text-accent-700 font-medium">View all</Link>
            </div>
            <div className="space-y-2">
              {orgs.map((o) => (
                <Link
                  key={o.id}
                  href={`/projectrequesta/organizations/${o.id}`}
        ...
        <div className="bg-white rounded-xl border border-slate-200/70 shadow-sm shadow-slate-200/60 p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/projectrequesta/projects" className="text-xs text-accent-600 hover:text-accent-700 font-medium">Browse all</Link>
          </div>
          <div className="space-y-2">
            {projects.slice(0, 5).map((p) => (
              <Link
                key={p.id}
                href={`/projectrequesta/projects/${p.id}`}
                className="flex items-center justify-between p-3 rounded-lg border border-slate-100 hover:border-accent-200 hover:bg-accent-50/40 transition-colors"
              >
                <div className="flex items-center gap-2.5">
