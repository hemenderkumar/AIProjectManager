...
type Dispute = {
  id: string;
  prProjectId: string | null;
  prAgreementId: string | null;
  description: string;
  status: string;
...
export default function ProjectRequestaDisputesPage() {
  ...
  async function load() {
    setLoading(true);
    const [disputesRes, meRes] = await Promise.all([fetch("/api/projectrequesta/disputes"), fetch("/api/projectrequesta/me")]);
  ...
  async function updateStatus(disputeId: string, status: string) {
    await fetch(`/api/projectrequesta/disputes/${disputeId}`, {
      method: "PATCH",
  ...
  return (
    <div>
      <Topbar title="Disputes" subtitle="Raised on a project or agreement, mediated by Executa's platform team" />
