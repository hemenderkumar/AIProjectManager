...
export default function VendorDirectoryPage() {
  ...
  async function load(f: typeof emptyFilters = filters) {
    ...
    const res = await fetch(`/api/projectrequesta/vendors${qs ? `?${qs}` : ""}`);
  ...
  return (
    <div>
      <Topbar title="Vendor Directory" subtitle="Search and filter verified outsourcing partners on ProjectRequesta" />
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {vendors.map((v) => (
                <Link
                  key={v.id}
                  href={`/projectrequesta/organizations/${v.id}`}
