...
export default function ProjectRequestaMfaPage() {
  ...
  async function load() {
    setLoading(true);
    const res = await fetch("/api/projectrequesta/mfa/status");
  ...
  async function startSetup() {
    setError(null);
    const res = await fetch("/api/projectrequesta/mfa/setup", { method: "POST" });
    if (!res.ok) {
  ...
  async function confirmSetup() {
    setError(null);
    const res = await fetch("/api/projectrequesta/mfa/verify", {
      method: "POST",
  ...
  async function disable() {
    setError(null);
    const res = await fetch("/api/projectrequesta/mfa/disable", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div>
      <Topbar title="Two-factor authentication" subtitle="Required for Client Finance Approver and Platform roles on ProjectRequesta" />
      <div className="p-8 max-w-lg space-y-6">
        {status?.mfaRequired && !status.mfaEnabled && (
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-xs text-amber-900">
            One of your ProjectRequesta roles requires MFA. You won&apos;t be able to use it until you set this up.
          </div>
        )}
