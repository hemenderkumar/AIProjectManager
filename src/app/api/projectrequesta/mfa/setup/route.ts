...
import { generateMfaSecret, buildOtpauthUrl, buildQrCodeDataUrl } from "@/lib/projectrequesta/mfa";
import { logAudit } from "@/lib/audit";

// Step 1 of enrollment: generate a fresh secret, store it (not yet enabled -- mfaEnabled
// stays false until POST /api/projectrequesta/mfa/verify confirms the user actually has it
// loaded in an authenticator app), and return a QR code to scan. Deliberately does NOT go
// through requirePrPlatform/requirePrOrgRole -- if it did, a Platform/Finance Approver user
// could never enroll in the first place, since those gates now require MFA to already be
// enabled. Any authenticated Executa user can call this; it has no effect until verified.
export async function POST() {
  ...
  await logAudit({ actor: user, action: "projectrequesta.mfa.setup_started", entityType: "user", entityId: user.id });
