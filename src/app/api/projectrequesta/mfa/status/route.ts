...
import { getPrMemberships, MFA_REQUIRED_ROLES } from "@/lib/projectrequesta/access";

// Drives the "set up MFA" banner in the UI: whether the account has MFA enabled, and
// whether any of the user's ProjectRequesta roles actually require it (Client Finance Approver,
// any Platform role). A user with mfaRequired=true and mfaEnabled=false will get a 403 from
// every route gated by requirePrPlatform/requirePrOrgRole the moment they try to use that
