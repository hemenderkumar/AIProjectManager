...
import { verifyMfaToken } from "@/lib/projectrequesta/mfa";
...
export async function POST(req: NextRequest) {
  ...
  if (!row?.mfaSecret) return NextResponse.json({ error: "Call /api/projectrequesta/mfa/setup first" }, { status: 400 });
  ...
  await logAudit({ actor: user, action: "projectrequesta.mfa.enabled", entityType: "user", entityId: user.id });
