...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ orgId: string }> }) {
  ...
  const ctx = await requirePrOrgRole(orgId, ["VENDOR_ORG_ADMIN"]);
  ...
  try {
    ...
    if (chargesEnabled !== org.stripeChargesEnabled || payoutsEnabled !== org.stripePayoutsEnabled) {
      await db.update(prOrganizations).set({ stripeChargesEnabled: chargesEnabled, stripePayoutsEnabled: payoutsEnabled }).where(eq(prOrganizations.id, orgId));
    }
...
// dashboard, tax forms, and payout schedule directly with Stripe -- Executa never touches
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ orgId: string }> }) {
  ...
  if (!accountId) {
    const account = await stripe.accounts.create({
      ...
      metadata: { prOrganizationId: org.id },
    });
    accountId = account.id;
    await db.update(prOrganizations).set({ stripeAccountId: accountId }).where(eq(prOrganizations.id, orgId));
    await logAudit({
      actor: ctx.user,
      action: "projectrequesta.organization.stripe_connected",
  ...
  const accountLink = await stripe.accountLinks.create({
    account: accountId,
    refresh_url: `${appUrl}/projectrequesta/organizations/${orgId}?stripe=refresh`,
    return_url: `${appUrl}/projectrequesta/organizations/${orgId}?stripe=return`,
