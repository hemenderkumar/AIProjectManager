...
import { prAgreements, prAgreementParties, prAgreementChangeRequests } from "@/lib/db/schema";
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ agreementId: string }> }) {
  ...
  const partyOrgIds = before.parties.map((p) => p.prOrganizationId).filter(Boolean) as string[];
  const myPartyOrgId = partyOrgIds.find((id) => memberships.some((m) => m.prOrganizationId === id)) ?? null;
  ...
  if (Object.keys(contentChanges).length && before.status === "ACTIVE" && !isPlatform) {
    const [changeRequest] = await db
      .insert(prAgreementChangeRequests)
      .values({
    ...
    await logAudit({
      actor: user,
      action: "projectrequesta.agreement.change_requested",
      entityType: "pr_agreement",
      entityId: agreementId,
      prOrganizationId: myPartyOrgId,
  ...
  const [updated] = await db.update(prAgreements).set(patch).where(eq(prAgreements.id, agreementId)).returning();

  await logAudit({
    actor: user,
    action: "projectrequesta.agreement.updated",
    entityType: "pr_agreement",
    entityId: agreementId,
    prOrganizationId: partyOrgIds[0] ?? null,
  ...
  if (body.status) {
    ...
    for (const orgId of otherPartyOrgIds) {
      notifyPrOrg(
        ...
        `An agreement you're a party to just moved to ${body.status}. Review it in ProjectRequesta.`
      ).catch(() => {});
    }
  }
