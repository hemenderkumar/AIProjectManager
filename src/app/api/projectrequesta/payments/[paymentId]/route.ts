...
import { prPayments, prMilestones, prAgreementParties, prOrganizations } from "@/lib/db/schema";
import { eq } from "drizzle-orm";
import { getCurrentUser } from "@/lib/auth";
import { canAccessPrPayment, requirePrPlatform } from "@/lib/projectrequesta/access";
...
// Platform Admin only -- Executa is the party actually moving money (or standing behind an
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ paymentId: string }> }) {
  ...
  const [before] = await db.select().from(prPayments).where(eq(prPayments.id, paymentId));
  ...
  if (body.status === "RELEASED" && before.stripePaymentIntentId && isStripeConfigured()) {
    const [milestone] = await db.select().from(prMilestones).where(eq(prMilestones.id, before.prMilestoneId));
    const parties = milestone ? await db.select().from(prAgreementParties).where(eq(prAgreementParties.prAgreementId, milestone.prAgreementId)) : [];
    const vendorOrgId = parties.find((p) => p.partyRole === "VENDOR")?.prOrganizationId;
    const [vendorOrg] = vendorOrgId ? await db.select().from(prOrganizations).where(eq(prOrganizations.id, vendorOrgId)) : [];
  ...
  const [updated] = await db.update(prPayments).set(patch).where(eq(prPayments.id, paymentId)).returning();

  if (body.status === "RELEASED") {
    await db.update(prMilestones).set({ status: "PAID" }).where(eq(prMilestones.id, before.prMilestoneId));
  }

  await logAudit({
    actor: ctx.user,
    action: "projectrequesta.payment.status_changed",
    ...
    if (milestone) {
      ...
      if (body.status === "RELEASED") {
        notifyPrOrg(
          vendorOrgId,
          "Payment released",
          `A payment of ${updated.currency} ${updated.amount.toLocaleString()} has been released to your organization on ProjectRequesta.`,
        ...
        notifyPrOrg(
          clientOrgId,
          "Payment refunded",
          `A payment of ${updated.currency} ${updated.amount.toLocaleString()} on ProjectRequesta has been refunded to your organization.`,
