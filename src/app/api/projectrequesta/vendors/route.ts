...
// posting a project -- deliberately open to any logged-in ProjectRequesta participant (not
// gated to Client roles only), since a Vendor should be able to see how they compare too.
//
// Rating aggregation: pr_reviews only links to pr_project_id, not directly to a vendor org,
// so a vendor's rating is "the average of every CLIENT-authored review on a project this
// vendor's org actually won" -- joined via the accepted (status=ACCEPTED) pr_bid for that
// project.
export async function GET(req: NextRequest) {
  ...
  const vendors = await db
    ...
    .where(and(eq(prOrganizations.orgType, "VENDOR"), eq(prOrganizations.isActive, true)));
