import { NextRequest, NextResponse } from "next/server";
import { db } from "@/lib/db";
import { prNegotiationEntries, prBids, prProjects } from "@/lib/db/schema";
import { eq } from "drizzle-orm";
import { getCurrentUser } from "@/lib/auth";
import { canAccessPrBid, getPrMemberships, clientOrgIds, vendorOrgIds, hasPlatformRole } from "@/lib/projectrequesta/access";
import { logAudit } from "@/lib/audit";
import { notifyPrOrg } from "@/lib/projectrequesta/notify";

export async function GET(_req: NextRequest, { params }: { params: Promise<{ bidId: string }> }) {
  const { bidId } = await params;
  const user = await getCurrentUser();
  if (!user) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  if (!(await canAccessPrBid(user, bidId))) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  const rows = await db
    .select()
    .from(prNegotiationEntries)
    .where(eq(prNegotiationEntries.prBidId, bidId))
    .orderBy(prNegotiationEntries.createdAt);
  return NextResponse.json(rows);
}

// Appends one counteroffer to the bid's negotiation thread, from whichever side (Client or
// Vendor) the caller belongs to, and updates the bid's own headline price/status to reflect
// the latest offer -- so prBids always shows "where things currently stand" while
// prNegotiationEntries preserves the full back-and-forth history. Only usable while the bid
// is still open to discussion (SUBMITTED or COUNTERED); once ACCEPTED/REJECTED the thread is
// closed.
export async function POST(req: NextRequest, { params }: { params: Promise<{ bidId: string }> }) {
  const { bidId } = await params;
  const user = await getCurrentUser();
  if (!user) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });

  const [bid] = await db.select().from(prBids).where(eq(prBids.id, bidId));
  if (!bid) return NextResponse.json({ error: "Not found" }, { status: 404 });
  if (bid.status !== "SUBMITTED" && bid.status !== "COUNTERED") {
    return NextResponse.json({ error: `Bid is ${bid.status}; negotiation is closed` }, { status: 400 });
  }
  const [project] = await db.select().from(prProjects).where(eq(prProjects.id, bid.prProjectId));
  if (!project) return NextResponse.json({ error: "Project not found" }, { status: 404 });

  const memberships = await getPrMemberships(user.id);
  const isPlatform = hasPlatformRole(memberships);
  const isClientSide = clientOrgIds(memberships).includes(project.clientOrgId);
  const isVendorSide = vendorOrgIds(memberships).includes(bid.vendorOrgId);
  if (!isPlatform && !isClientSide && !isVendorSide) {
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  }

  const body = await req.json().catch(() => ({}));
  const price = Number(body.price);
  if (!price || Number.isNaN(price)) {
    return NextResponse.json({ error: "price must be a positive number" }, { status: 400 });
  }

  const proposedByOrgType = isVendorSide && !isClientSide ? "VENDOR" : "CLIENT";

  const [entry] = await db
    .insert(prNegotiationEntries)
    .values({
      prBidId: bidId,
      price,
      currency: body.currency || bid.currency,
      terms: body.terms || null,
      proposedByUserId: user.id,
      proposedByOrgType,
    })
    .returning();

  const [updatedBid] = await db
    .update(prBids)
    .set({ status: "COUNTERED", proposedPrice: price, currency: body.currency || bid.currency, updatedAt: new Date() })
    .where(eq(prBids.id, bidId))
    .returning();

  await logAudit({
    actor: user,
    action: "projectrequesta.negotiation.entry_added",
    entityType: "pr_negotiation_entry",
    entityId: entry.id,
    prOrganizationId: isVendorSide && !isClientSide ? bid.vendorOrgId : project.clientOrgId,
    afterValue: JSON.stringify({ entry, updatedBid }),
  });

  // Notify whichever side didn't just make this move.
  if (proposedByOrgType === "VENDOR") {
    notifyPrOrg(
      project.clientOrgId,
      `Counteroffer on "${project.title}"`,
      `The vendor countered at ${entry.currency} ${entry.price.toLocaleString()} on "${project.title}". Review it in ProjectRequesta.`,
      ["CLIENT_ORG_ADMIN", "CLIENT_REQUESTER"]
    ).catch(() => {});
  } else {
    notifyPrOrg(
      bid.vendorOrgId,
      `Counteroffer on "${project.title}"`,
      `The client countered at ${entry.currency} ${entry.price.toLocaleString()} on "${project.title}". Review it in ProjectRequesta.`,
      ["VENDOR_ORG_ADMIN", "VENDOR_CONTRIBUTOR"]
    ).catch(() => {});
  }

  return NextResponse.json({ entry, bid: updatedBid }, { status: 201 });
}
