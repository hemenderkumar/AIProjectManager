...
import { prProjects } from "@/lib/db/schema";
import { getCurrentUser } from "@/lib/auth";
import { getPrMemberships, rolesInOrg, listPrProjectsForUser } from "@/lib/projectrequesta/access";
...
// See listPrProjectsForUser for the exact visibility rule. Search/filter (#255) is applied
...
export async function GET(req: NextRequest) {
  ...
  const projects = await listPrProjectsForUser(user);
...
export async function POST(req: NextRequest) {
  ...
  await logAudit({
    actor: user,
    action: "projectrequesta.project.created",
