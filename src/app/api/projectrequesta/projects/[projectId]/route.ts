...
import { prProjects, prAgreements } from "@/lib/db/schema";
import { eq } from "drizzle-orm";
import { getCurrentUser } from "@/lib/auth";
import { canAccessPrProject, getPrMemberships, rolesInOrg, hasPlatformRole } from "@/lib/projectrequesta/access";
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ projectId: string }> }) {
  ...
  const [before] = await db.select().from(prProjects).where(eq(prProjects.id, projectId));
  ...
  if (body.status) {
    ...
    if (body.status === "IN_PROGRESS" && !isPlatform) {
      const projectAgreements = await db.select().from(prAgreements).where(eq(prAgreements.prProjectId, projectId));
  ...
  const [updated] = await db.update(prProjects).set(patch).where(eq(prProjects.id, projectId)).returning();

  await logAudit({
    actor: user,
    action: "projectrequesta.project.updated",
    entityType: "pr_project",
    entityId: projectId,
    prOrganizationId: before.clientOrgId,
