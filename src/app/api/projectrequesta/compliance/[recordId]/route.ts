...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ recordId: string }> }) {
  ...
  const [before] = await db.select().from(prComplianceRecords).where(eq(prComplianceRecords.id, recordId));
  ...
  const [updated] = await db
    ...
    .where(eq(prComplianceRecords.id, recordId))
    .returning();

  await logAudit({
    actor: ctx.user,
    action: "projectrequesta.compliance_record.decided",
    entityType: "pr_compliance_record",
    entityId: recordId,
    prOrganizationId: before.prOrganizationId,
