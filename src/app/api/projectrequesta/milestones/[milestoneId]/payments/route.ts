...
import { prPayments, prMilestones, prAgreementParties } from "@/lib/db/schema";
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ milestoneId: string }> }) {
  ...
  if (!(await canAccessPrAgreement(user, milestone.prAgreementId))) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  return NextResponse.json(await db.select().from(prPayments).where(eq(prPayments.prMilestoneId, milestoneId)));
}
...
// since Executa itself controls that money movement. Every payment starts PENDING regardless of
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ milestoneId: string }> }) {
  ...
  await logAudit({
    actor: user,
    action: "projectrequesta.payment.created",
