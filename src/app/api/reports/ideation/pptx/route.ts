...
export async function POST() {
  ...
  const buffer = await generateTablePptx("Ideation", `${rows.length} ideas in progress`, columns, rows, generatedAt);

  return new NextResponse(new Uint8Array(buffer), {
    headers: {
      "Content-Type": "application/vnd.openxmlformats-officedocument.presentationml.presentation",
      "Content-Disposition": `attachment; filename="ideation.pptx"`,
