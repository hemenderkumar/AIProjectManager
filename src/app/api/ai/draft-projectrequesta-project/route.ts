...
type ProjectRequestaProjectDraft = {
  title: string;
  description: string;
  category: string;
  targetBudget: number;
...
// ProjectRequesta's own "rough note -> structured record" AI drafting endpoint -- the same
// pattern Executa already uses for tasks/risks/etc. (see draft-task/route.ts), but
...
// the engagement should run direct (marketplace) or Executa-mediated on both sides.
//
// Optionally seeded from an existing Executa task (see the post-to-projectrequesta bridge
...
// it stays usable standalone from the ProjectRequesta "Post a project" form too.
export async function POST(req: NextRequest) {
  ...
  } to ProjectRequesta, a B2B marketplace where vetted Vendor organizations ${
...
by default, or "MEDIATOR" only if the note implies wanting Executa itself as the contracting party on both
...
"FIXED" -- whichever the note implies, defaulting to "HOURLY" if unclear`
  ...
  const { data, error } = await askClaudeJSON<ProjectRequestaProjectDraft>(system, "Draft this project posting now.", 1200);
