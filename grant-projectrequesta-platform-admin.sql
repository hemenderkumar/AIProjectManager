-- Bootstraps ProjectRequesta's first PLATFORM_ADMIN. Nothing in the app currently grants this
-- role automatically (unlike Executa's ADMIN/SUPER_USER, ProjectRequesta's PLATFORM_*
-- roles have no seed step), so until this runs once, nobody can open /projectrequesta/admin,
...
-- in as the account below and set up MFA at /projectrequesta/mfa -- PLATFORM_ADMIN is one of
-- the roles that requires it (see MFA_REQUIRED_ROLES in src/lib/projectrequesta/access.ts),
